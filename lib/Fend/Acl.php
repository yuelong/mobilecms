 ...
 * @version $Id: Acl.php 3 2011-12-29 15:01:09Z gimoo $
 */
...
class Fend_Acl extends Fend
{
    ...
    public function run($app)
    {
        ...
        // 检测是否需要进入候补状态
        if ($this->aclcfg['isDef'] && $module!=$this->aclcfg['deMod'] && !empty($this->_module) && !isset($this->_module[$module]) ) {
            $action=$controller;
            $controller=$module;
            $module=$this->aclcfg['deMod'];
        }
    ...
    private function getParam(&$app)
    {
        ...
        foreach ($app as $k => $v) {
            if (strpos($v, '.')) {
                $app[$k] = strstr(strtolower($v), '.', TRUE);
            }
        }
        $this->url=$app;
    }
