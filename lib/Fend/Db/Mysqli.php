 ...
 * @version $Id: Mysqli.php 94 2012-04-06 14:26:11Z gimoo $
 */
...
class Fend_Db_Mysqli extends Fend
{
    ...
    public function getCount($sql,$exp=86400)
    {
        $sql=trim($sql);
        $cacheID='DBA:'.md5($sql);
        $total = $exp<=0 ? 0 : Fend_Cache::get($cacheID);
        if ( $exp<=0 || ($total<=0 && !Fend_Cache::isKey($cacheID)) ) {
            ...
            // 默认缓存一天
            $exp>0 && Fend_Cache::set($cacheID,$total,$exp);
        }
    ...
    {
        if($this->dbError){
            throw new Fend_Exception($str.$this->_db->error);
        }elseif(defined('FD_DBLOG')){
            $_tmp='';
            isset($_SERVER['SERVER_ADDR']) && $_tmp.='['.$_SERVER['SERVER_ADDR'].']';
            isset($_SERVER['REQUEST_URI']) && $_tmp.='['.$_SERVER['REQUEST_URI'].']';
            $_tmp && $_tmp.="\n";

            file_put_contents(FD_DBLOG,date("Y-m-d H:i:s > ").$_tmp.$str.$this->_db->error."\n", FILE_APPEND );
        }
    }
}
?>